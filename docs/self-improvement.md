...
Relentless has completed its first bounded self-improvement cycle on its own failure classifier. The models ran through Relentless's actual CLI and Pi adapter; the outer coding assistant supplied source snapshots, selected/reproduced the finding, applied the patch and ran checks. This is assisted self-improvement, not an autonomous self-modifying daemon.

## Cycle 001: authentication evidence takes precedence

1. Qwen3.8 Flash reviewed `src/failures.ts` and its existing tests using `relentless swarm`. It identified that `fromProviderError({code: "rateLimitExceeded", message: "401 Unauthorized"})` returned `quota` instead of `auth`.
...
4. A separate GPT-5.6 Luna worker through Relentless reviewed the final source/tests and the original finding. It returned `pass` with no findings. The final reviewed source was checked against the review request and hashed locally.
...
The discovery worker incorrectly described the impact as infinite retries; Relentless already bounds attempts. That claim was rejected. Its broad suggested merge of all unknown errors was also not adopted. Model output remained evidence, and deterministic reproduction established the actual defect.
...
Cycle 002 now uses Relentless coding workers to author and repair its own evidence ranker, with external integration/gates and independent reviews. See the [active program](self-improvement-program.md) and [measured routing](model-efficiency.md).
