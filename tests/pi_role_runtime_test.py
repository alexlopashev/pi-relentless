...
const root=await mkdtemp(join(tmpdir(),'clanker-role-runtime-'));
await mkdir(join(root,'.pi'));
await writeFile(join(root,'.pi/settings.json'),JSON.stringify({clanker:{version:1,routing:{maxConcurrency:1,candidates:[{name:'local',provider:'clanker-local',model:'qwen3.5-4b',billing:'local',enabled:true,quality:1,preference:1,efforts:['off']}]},roles:{scheduler:['local']}}}));
const loaded=await discoverAndLoadExtensions([resolve('.pi/extensions/clanker.ts'),resolve('.pi/extensions/local-provider.ts')],root,join(root,'agent'));
...
await runtime.getAvailable('clanker-local');
const registry=new ModelRegistry(runtime);
const local=registry.find('clanker-local','qwen3.5-4b');
if(!local)throw Error('Local provider missing from Pi');
const command=loaded.extensions.flatMap(e=>[...e.commands.values()]).find(c=>c.name==='clanker');
...
if(selected.provider!=='clanker-local'||selected.effort!=='off'||selected.dispatched!==false)throw Error('Unexpected selection');
