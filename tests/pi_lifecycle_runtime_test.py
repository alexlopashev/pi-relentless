...
const extension=loaded.extensions.find(e=>e.commands.has('relentless'));
const command=extension?.commands.get('relentless');
...
const settings={relentless:{version:1,routing:{candidates:[candidate]},roles:{coder:['local']},resumeGoal:{id,revision:1}}};
...
await extension.commands.get('relentless').handler('pause',ctx);
